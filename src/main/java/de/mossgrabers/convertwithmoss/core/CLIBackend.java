...
public class CLIBackend implements INotifier
{
    ...
    public void parseCommandLine (final String [] arguments)
    {
        try
        {
            ...
            spec.addOption (OptionSpec.builder ("-Zlc", "--ProcessLoopCrossfadeClicking").paramLabel ("PROCESS_LOOP_CROSSFADE_CLICKING").type (Boolean.class).description ("Sets the loop crossfade of -Zl only on forward loops which still click at their wrap after snapping with -Zs, if enabled - loops which the snapping fixed get none - limited to the audio in front of the loop start, if processing is enabled.").build ());
