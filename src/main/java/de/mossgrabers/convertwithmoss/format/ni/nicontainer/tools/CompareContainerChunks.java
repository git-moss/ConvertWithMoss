...
public class CompareContainerChunks
{
    ...
    public static void main (final String [] args)
    {
        if (args.length != 2)
        {
            IO.println ("Two absolute paths of the files to compare need to be given as the first and second parameter.");
            return;
        }
    ...
    private static void compare (final NIContainerItem niContainerItem1, final NIContainerItem niContainerItem2, final int level)
    {
        ...
        while (dataChunk1 != null && dataChunk2 != null)
        {
            ...
            {
                IO.println (StringUtils.padLeftSpaces ("Different Item Content: ", depth) + data1.getClass ().getName () + data2.getClass ().getName ());
                return;
            }
            IO.println (StringUtils.padLeftSpaces ("Item Content: ", depth) + data1.getClass ().getSimpleName ());
        ...
        {
            IO.println (StringUtils.padLeftSpaces ("Different number of data chunks!", depth));
            return;
        }

        if (Arrays.compare (niContainerItem1.getUUID (), niContainerItem2.getUUID ()) != 0)
        {
            IO.println (StringUtils.padLeftSpaces ("Different UUID:", depth));
            IO.println (StringUtils.padLeftSpaces ("  " + StringUtils.formatHexStr (niContainerItem1.getUUID ()), depth));
            IO.println (StringUtils.padLeftSpaces ("  " + StringUtils.formatHexStr (niContainerItem2.getUUID ()), depth));
        }
        if (niContainerItem1.getVersion () != niContainerItem2.getVersion ())
            IO.println (StringUtils.padLeftSpaces (String.format ("Different versions: %d %d ", Integer.valueOf (niContainerItem1.getVersion ()), Integer.valueOf (niContainerItem2.getVersion ())), depth));
        ...
        {
            IO.println (StringUtils.padLeftSpaces ("Different number of children!", depth));
            return;
        }
        ...
        IO.println (StringUtils.padLeftSpaces ("Children:", depth));
        for (int i = 0; i < children1.size (); i++)
        {
            ...
            if (!domainID1.equals (domainID2))
                IO.println (StringUtils.padLeftSpaces (String.format ("Different domains: %s %s ", domainID1, domainID2), depth));
            else
                IO.println (StringUtils.padLeftSpaces ("* Domain: " + domainID1, depth));
            ...
            if (chunkTypeID1 != chunkTypeID2)
                IO.println (StringUtils.padLeftSpaces (String.format ("Different types: %s %s ", NIContainerChunkType.get (chunkTypeID1), NIContainerChunkType.get (chunkTypeID2)), depth));
            else
                IO.println (StringUtils.padLeftSpaces ("* Type: " + NIContainerChunkType.get (chunkTypeID1), depth));
    ...
    private static void compareData (final IChunkData data1, final IChunkData data2, final int level)
    {
        ...
        IO.println (StringUtils.padLeftSpaces ("Data 1:", level * 4));
        IO.print (data1.dump (level + 1));
        IO.println (StringUtils.padLeftSpaces ("Data 2:", level * 4));
        IO.print (data2.dump (level + 1));
    }
}
