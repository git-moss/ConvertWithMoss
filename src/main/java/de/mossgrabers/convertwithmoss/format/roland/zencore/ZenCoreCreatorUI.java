...
public class ZenCoreCreatorUI implements ICoreTaskSettings
{
     ...
     * flag bytes differ per device - a ZENOLOGY file uses 03 03 / 01, not the FANTOM's 05 04 / 24 - so
     * the whole header, not only the model tag, is selected.
     */
    private static final byte [] []  HEADERS             =
    {
        ...
        // ZENOLOGY plug-in (device-confirmed from an export)
        {
            'S', 'V', 'Z', 'a', 0x03, 0x03, 'R', 'C', '0', '0', '1', 0x01, 0, 0, 0, 0
        }
    };
