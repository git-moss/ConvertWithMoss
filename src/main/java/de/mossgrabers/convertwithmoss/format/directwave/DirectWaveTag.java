...
public class DirectWaveTag
{
    ...
    protected static final byte []  MAGIC                   = "DwPr".getBytes ();
     ...
     * reproduces all measured positions exactly and predicts the 2.52 Hz which the two depth probes
     * show at their knob position of 0.355.
     */
    ...
    /**
     * The modulation amount is bipolar: this value is no modulation at all, the distance from it is
     * the strength and the side is the sign. Measured on all eight probe zones, which are symmetric
     * around it.
     */
    public static final double      MODULATION_NEUTRAL      = 0.5;

    /**
     * The pitch modulation in cent at the full modulation strength. The strength is the cube of the
     * distance from the neutral amount; measured at four amounts, which the law
    ...
    protected static final byte []  FLAC_MAGIC              = "fLaC".getBytes ();
     ...
     * <code>u32 waveform, f32 rate, 3 unknown floats</code>. An LFO does nothing on its own, it has
     * to be routed to a target in the modulation matrix.
     */
     ...
     * mapped with the taper {@link #ENVELOPE_TIME_EXPONENT}. Both values were measured by playing a
     * program whose zones differ only in the decay knob against reference zones whose fade-out is
     * part of their audio; they reproduce the six measured decay times within 3 %.
     */
    ...
    protected static final float [] ENVELOPE_DEFAULTS_V25   =
    {
        0f,
        0.5f,
        1f,
    ...
    protected static final float [] ENVELOPE_DEFAULTS_V26   =
    {
        0f,
        1f,
        1f,
