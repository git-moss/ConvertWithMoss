...
public class EmulatorXConstants
{
    ...
    /**
     * Convert a normalized filter cutoff to a frequency in Hertz. The curve is exponential from ~57
     * Hz at 0 to 20 kHz at 1.
     *
     ...
     */
    public static float hertzToCutoff (final Optional<IFilter> filterOpt, final int filterType)
    {
        if (filterOpt.isEmpty () || filterType == EmulatorXConstants.FILTER_TYPE_BYPASS)
            return 1.0f;
        final double limited = Math.clamp (filterOpt.get ().getCutoff (), CUTOFF_MIN_HERTZ, CUTOFF_MAX_HERTZ);
        return (float) Math.clamp (Math.log (limited / CUTOFF_MIN_HERTZ) / Math.log (CUTOFF_MAX_HERTZ / CUTOFF_MIN_HERTZ), 0, 1);
    }
