 ...
 * <p>
 * The record is the runtime structure of the voice: the operating system keeps most settings in the
 * form the playback engine uses - tables with one entry per velocity zone (level, cutoff, Q, VCA
 * attack and VCF attack), the counters of the envelope stages, and pointers into the tables of the
 * LFO depths. The meaning of the bytes and the laws were established by aligning 9,563 voice
...
class Emulator2VoiceSettings
{
    ...
    /**
     * The counters of the decay and release stages: the number of passes and the negative
     * increment.
     */
     ...
     *
     * @param zone The zone
     * @param voiceRecord The 256 bytes of the voice record
     */
    static void apply (final ISampleZone zone, final byte [] voiceRecord)
    {
        // Tuning
        zone.setTuning (voiceRecord[FINE_TUNE] * FINE_TUNE_CENTS / 100.0);
        ...
        final int level = voiceRecord[LEVEL_TABLE] & 0xFF;
        zone.setGain (-lookup (ATTENUATION_DB, level));
        final double velocityToLevel = lookup (VELOCITY_LEVEL_DB, level - (double) (voiceRecord[LEVEL_TABLE + TABLE_LAST_ENTRY] & 0xFF));
        ...
        amplitudeEnvelope.setAttackTime (attackTime (voiceRecord[VCA_ATTACK_HIGH] & 0xFF, voiceRecord[VCA_ATTACK_LOW] & 0xFF));
        amplitudeEnvelope.setDecayTime (decayTime (voiceRecord[VCA_DECAY_HIGH] & 0xFF, voiceRecord[VCA_DECAY_LOW] & 0xFF));
        amplitudeEnvelope.setSustainLevel (Math.clamp (1 - lookup (VCA_SUSTAIN_DB, voiceRecord[VCA_SUSTAIN] & 0xFF) / 100.0, 0, 1));
        amplitudeEnvelope.setReleaseTime (decayTime (voiceRecord[VCA_RELEASE_HIGH] & 0xFF, voiceRecord[VCA_RELEASE_LOW] & 0xFF));
        amplitudeEnvelope.setTimeVelocityTracking (((voiceRecord[VELOCITY_TO_ATTACK] & 0xFF) >> 4) / 15.0);

        // LFO: one triangle LFO which the voice routes to the pitch, the filter and the level
        final double lfoRate = lfoRate (voiceRecord[LFO_RATE] & 0xFF);
        final double lfoDelay = Math.max (0, (voiceRecord[LFO_DELAY] & 0xFF) - 1) / 100.0;
        final double lfoToPitch = lookup (LFO_PITCH_CENTS, lfoTableIndex (voiceRecord, LFO_TO_PITCH));
        if (lfoToPitch > 0)
        {
        ...
        final double lfoToLevel = lookup (LFO_LEVEL_DB, lfoTableIndex (voiceRecord, LFO_TO_LEVEL));
        if (lfoToLevel > 0)
        {
        ...
        final int cutoff = voiceRecord[CUTOFF_TABLE] & 0xFF;
        final double velocityToCutoff = VELOCITY_CUTOFF_CENTS_PER_UNIT * (cutoff - (voiceRecord[CUTOFF_TABLE + TABLE_LAST_ENTRY] & 0xFF));
        final int amountValue = voiceRecord[ENVELOPE_AMOUNT] & 0xFF;
        double envelopeAmount = amountValue == ENVELOPE_AMOUNT_NONE ? 0 : lookup (ENVELOPE_AMOUNT_CENTS, amountValue);
        if ((voiceRecord[FLAGS] & FLAG_NEGATIVE_ENVELOPE) != 0)
            envelopeAmount = -envelopeAmount;
        final double lfoToFilter = lookup (LFO_FILTER_CENTS, lfoTableIndex (voiceRecord, LFO_TO_FILTER));
        ...
        final int q = voiceRecord[Q_TABLE] & 0xFF;
        ...
        filter.setCutoffKeyTracking (Math.min (1, ((voiceRecord[KEY_TRACKING] & 0xFF) >> 4) / 8.0));
        ...
        if (envelopeAmount != 0)
        {
            ...
            filterEnvelope.setAttackTime (vcfTime (attackTime (voiceRecord[VCF_ATTACK_HIGH] & 0xFF, voiceRecord[VCF_ATTACK_LOW] & 0xFF)));
            filterEnvelope.setDecayTime (vcfTime (decayTime (voiceRecord[VCF_DECAY_HIGH] & 0xFF, voiceRecord[VCF_DECAY_LOW] & 0xFF)));
            filterEnvelope.setSustainLevel (lookup (VCF_SUSTAIN_LEVEL, Math.min (voiceRecord[VCF_SUSTAIN] & 0xFF, 0x1F)));
            filterEnvelope.setReleaseTime (vcfTime (decayTime (voiceRecord[VCF_RELEASE_HIGH] & 0xFF, voiceRecord[VCF_RELEASE_LOW] & 0xFF)));
            ...
            filterEnvelope.setTimeVelocityTracking ((voiceRecord[VELOCITY_TO_ATTACK] & 0x0F) / 15.0);
        }
        if (lfoToFilter > 0)
        {
     ...
     */
    private static int lfoTableIndex (final byte [] voiceRecord, final int offset)
    {
        final int page = voiceRecord[offset + 1] & 0xFF;
        if (page < LFO_TABLE_PAGE)
            return 0;
        return (page - LFO_TABLE_PAGE) * 4 + (voiceRecord[offset] & 0xFF);
    }


    /**
     * The time of an attack stage: the counter increments a 255 step ramp by the low value every 10
     * ms, the high value times.
     *
