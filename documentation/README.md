...
2. Now the source file(s) need to be selected. There are 2 conversion modes: 1) Convert several source files (batch mode) or 2) one single source file. Select either "Folder" for batch mode or "File" for single file conversion with the choice box in front of the source text field.
3. Select the source folder/file with the *Select* button (or type/copy it into the text field). In the default batch mode, the files can also be located in sub-folders.
   * In single file mode, *Select* opens a file dialog whose filter box lists every source format with its file endings, the current one preselected. Afterwards the source format is set from the picked file: a filter which you switched in the dialog wins, otherwise the file ending decides, and a disk image goes to the generic ISO/IMG format, which hands it to whichever sampler format can read it.
   * Each mode keeps a path history of its own, which is exchanged along with the entered path when you use the toggle. You can also simply type the path of a file into the field, which converts that file alone whatever the toggle says.
4. Select the destination format on the right.
5. Select the output folder where you want to create the multisamples. You can add a non-existing folder to the name, which then is automatically created. E.g. you could select the Desktop and then add a folder *Conversions*.
...
Every source format has a *Contents...* button. It reads the source without writing anything and shows all presets it found as a tree of their folders, of their file - if the file contains more than the one preset - and of the containers inside of that file (e.g. image, bank, preset). Each entry shows its number of zones, its key range and its category, which is useful to explore a bank or a preset folder you do not know yet. A preset which shares its name with another one in the same folder additionally shows the index which addresses it inside its file (e.g. *#12*), since library CD-ROMs do re-use a name for a variant of a preset; it is the same index the exported list writes. Tick the presets you want and only those are converted; the number of them is shown below the source path and *Clear* discards the selection again.
...
* **Reduce sample frequency**: Reduces the sample frequency of all samples to the given value. If the sample frequency is smaller than the selected value the sample is not modified. A loop stays as seamless as it is in the source: it is converted as the repeating audio it is and stretched to a whole number of frames, which changes its pitch by less than a quarter of a cent (for a loop of 4096 frames or more). The same applies when a destination format converts the sample frequency itself. The conversion overshoots at steep edges, e.g. of square waves, therefore a sample which peaks close to full scale would clip: it is lowered to fit and its zone plays it louder by the same amount.
* **Always re-sample**: Does as well up-sampling to the set sample frequency and bit depth, if enabled.
* **Snap loops which click**: Moves the start and end of forward loops which click at the loop point, which some sample libraries have (e.g. auto-sampled instruments whose loop was not designed to be click-free). The wrap of every loop is measured first and a loop which already wraps cleanly is left untouched, as well as a loop with a cross-fade and a single-cycle loop - moving the boundaries of a clean loop can only break it. For a loop which clicks, a nearby zero-crossing and the position where the waveform continues best are tried, and the one with the smaller click is taken if it reduces the click clearly. A position which jumps at the loop point is not taken for a loop which does not jump there, even if the curve of the waveform is smoother: e.g. a loop whose last frame repeats its first frame bends a little at the loop point, while the nearest zero-crossings of a pulse wave can be half of its level apart. The log counts the loops which were moved - a moved loop clicks less, but not necessarily not at all; the note about clicking loops names the loops which still click. Enabled with `-Zs` on the command line.
* **Cross-fade loops**: Sets all loop cross-fades (if supported by the destination format) to this percentage value, or only those of the loops which still click, see below. For a destination whose sound engine cannot cross-fade at playback (e.g. Roland ZEN-Core), the cross-fade is instead baked into the written sample audio; a loop that already wraps cleanly is left untouched.
* **Only loops which still click**: Sets the loop cross-fade above only on forward loops which still click at the loop point after *Snap loops which click* - the loops which the note about clicking loops reports - and leaves all other loops as they are, including their own cross-fades: a loop which snapping fixed gets no cross-fade. Without snapping, the cross-fade is set on all loops which click. A loop counts as clicking when the jump at its loop point is large compared with the movement of the waveform around it and the loop point also breaks the curve of the waveform - the measure with which *Snap loops which click* decides which loops to move - so a loop which wraps seamlessly through a steep part of the waveform, e.g. at the edge of a saw wave, is neither reported nor cross-faded. A loop which has a cross-fade of its own is never reported, since that cross-fade already smooths the wrap. The percentage is the maximum: a cross-fade blends the end of the loop with the audio in front of the loop start, therefore it is limited to the audio which is there; a loop which starts at the very beginning of its sample cannot get one, and the note about clicking loops names such loops. The option is available when a cross-fade above 0% is selected. Enabled with `-Zlc` together with `-Zl` on the command line.
...
* **Transpose: Auto**: Instead of a fixed number of semitones, the pitch of the samples of each multi-sample is measured and its root notes are moved onto the nearest semitone of it, so that every preset of a library plays at the pitch of the keys - a preset which sounds two octaves up and one which is in tune are each corrected by what they need. The pitch of a sample is the highest fundamental whose harmonics hold most of the power of its spectrum, measured in up to twelve zones spread over the multi-sample; a multi-sample is only transposed if at least three of them could be measured and at least 80% of them agree, so drums, sound effects and phrase kits whose zones are each rooted differently are left alone. Only the root notes are moved: the audio, the key ranges and the tuning of the zones stay as they are. Enabled with `-Zpa` on the command line, where it overrides `-Zp`. Whether a transposition is wanted is a decision and not a defect: an instrument is often played one or two octaves away from the key - a bass patch, an oscillator at another footage, an operator at the frequency ratio of one half or two - and the measurement cannot tell that apart from a mapping which is simply wrong. The option is therefore meant for the presets which need it rather than for a whole library at once, where it would move the bass patches as well. The note about the pitch of the samples is logged without the option, so a conversion which is run without it already reports which presets sound away from their keys ('Note: the samples of ... sound +23.8 semitones from their root keys'); those can then be converted again with the option. Every multi-sample which the option transposes is logged with the number of semitones it was moved by, so what it did can be checked afterwards. A multi-sample which the measurement leaves without a result is not transposed at all - if such a preset does sound away from its keys, use the fixed transposition above.

## Options

* **Create folder structure**: If enabled, sub-folders from the source folder are created as well in the output folder. For example, if I select my whole "Sounds" folder, there are sub-folders like `Sounds/07 Synth/Lead/01W Emerson'70 Samples`. In that case the output folder would contain e.g. `07 Synth/Lead/01W Emerson'70.multisample` if Bitwig multisample is selected as the destination format. A folder which a format treats as one file, e.g. the dsbundle of DecentSampler, is not reproduced: its presets go into the folder which contains it, like the presets of a library file.
