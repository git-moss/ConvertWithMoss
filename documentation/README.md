...
* **Snap loops to zero-crossings**: Moves the start and end of forward loops which click at the loop point, which some sample libraries have (e.g. auto-sampled instruments whose loop was not designed to be click-free). The wrap of every loop is measured first and a loop which already wraps cleanly is left untouched, as well as a loop with a cross-fade and a single-cycle loop - moving the boundaries of a clean loop can only break it. For a loop which clicks, a nearby zero-crossing and the position where the waveform continues best are tried, and the one with the smaller click is taken if it reduces the click clearly. Enabled with `-Zs` on the command line.
