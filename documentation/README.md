...
A source format where one file contains several presets - e.g. a bank, a disk image or a library - additionally has a *Contents...* button. It reads the source without writing anything and shows all presets it found as a tree of their file and their containers (e.g. image, bank, preset). Each entry shows its number of zones, its key range and its category, which is useful to explore a bank you do not know yet. Tick the presets you want and only those are converted; the number of them is shown below the source path and *Clear* discards the selection again.
