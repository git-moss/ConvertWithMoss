...
* **Snap loops which click**: Moves the start and end of forward loops which click at the loop point, which some sample libraries have (e.g. auto-sampled instruments whose loop was not designed to be click-free). The wrap of every loop is measured first and a loop which already wraps cleanly is left untouched, as well as a loop with a cross-fade and a single-cycle loop - moving the boundaries of a clean loop can only break it. For a loop which clicks, a nearby zero-crossing and the position where the waveform continues best are tried, and the one with the smaller click is taken if it reduces the click clearly. Enabled with `-Zs` on the command line.
* **Only loops which click**: Sets the loop cross-fade above only on forward loops which click at the loop point - the loops which the note about clicking loops reports - and leaves all other loops as they are, including their own cross-fades. A loop which has a cross-fade of its own is never reported, since that cross-fade already smooths the wrap. It is applied after the loops were snapped to zero-crossings, if that is enabled, so only the loops which snapping could not fix get a cross-fade. A cross-fade blends the end of the loop with the audio in front of the loop start, therefore it is limited to the audio which is there; a loop which starts at the very beginning of its sample cannot get one. Enabled with `-Zlc` together with `-Zl` on the command line.
