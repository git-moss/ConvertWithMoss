...
* **Transpose: Auto**: Instead of a fixed number of semitones, the pitch of the samples of each multi-sample is measured and its root notes are moved onto the nearest semitone of it, so that every preset of a library plays at the pitch of the keys - a preset which sounds two octaves up and one which is in tune are each corrected by what they need. The pitch of a sample is the highest fundamental whose harmonics hold most of the power of its spectrum, measured in up to twelve zones spread over the multi-sample; a multi-sample is only transposed if at least three of them could be measured and at least 80% of them agree, so drums, sound effects and phrase kits whose zones are each rooted differently are left alone. Only the root notes are moved: the audio, the key ranges and the tuning of the zones stay as they are. Enabled with `-Zpa` on the command line, where it overrides `-Zp`.
