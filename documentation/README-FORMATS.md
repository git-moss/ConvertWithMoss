...
* [Kurzweil PC3/Forte](#kurzweil-pc3forte)
...
The Decent Sampler plugin is a free (but closed source) sample player plugin that allows you to play sample libraries in the DecentSampler format (files with extensions: dspreset, dsproduct and dslibrary). See https://www.decentsamples.com/product/decent-sampler-plugin/
...
A dspreset file contains a single preset and the description of the multi-sample. A dsproduct file contains the same description in a binary container; version 001 of it can be read but is not written. The related samples are normally kept in a separate folder, which is searched relative to the preset file, therefore keep the folder structure of a library as it is. WAV, AIFF and FLAC files are supported. A dslibrary file contains dspreset or dsproduct files including the samples compressed in ZIP format. A dsbundle is similar but uncompressed. Both are treated as one file when the folder structure is created in the output folder: their presets go into the folder which contains the library or the bundle. A dsproduct file of another version, a truncated or damaged dsproduct file and a preset with invalid XML are reported; the other presets of a library are still read.

The conversion reads the sound which DecentSampler plays after loading a preset. DecentSampler sends the stored values of the knobs, sliders, menus and buttons of the user interface to the parameters which they are bound to when it loads a preset, therefore these values are applied (with linear, table and fixed-value translations): the amplitude envelope, volume, pan, tuning, velocity and key tracking, switching groups and tags on and off, the volume and polyphony of tags, the frequency and resonance of filters, the level of the gain effect and the amount and rate of modulators. The settings of the groups element and of a group are the defaults of their samples. The volumes of the instrument, the group, the sample, its tags, the gain effects and the main output are combined. Groups and samples which are disabled or carry a disabled tag are not converted, except with 'Create one multi-sample per group'. A sample whose amplitude envelope is switched off (ampEnvEnabled) plays as a one-shot. Samples which silence each other with a tag (silencedByTags), e.g. an open and a closed hi-hat which both carry and are silenced by the tag 'hihat', become an exclusive group. If a preset does not set any parameter of the amplitude envelope, the default envelope of its category is applied, like for other formats without an envelope; otherwise, a curve which is not set gets the default of DecentSampler, a logarithmic attack and an exponential decay and release.

The user interface itself and MIDI controller bindings are not converted, the converted preset sounds like the stored state of the controls. The following features change the sound but cannot be converted, they are listed in a message for each preset which uses them: effects other than the filter and the gain effect (e.g. a convolution or reverb), more than one filter in an effect chain, effect buses, outputs other than the main output, the arpeggiator, note sequences, silencing by tags which is not mutual (e.g. a closed hi-hat which stops the open one but not vice versa), fading out silenced samples and the curve of loop cross-fades (their length is kept). The destination format can limit further what is kept, e.g. one-shots and exclusive groups. See [DecentSampler Product Presets](design/DECENTSAMPLER_PRODUCT_FORMAT.md) for the details and how the import was validated.
...
The E-mu Emulator II (1984) stores a *bank* - up to 100 voices, each a sample with its loop and its settings, and up to 100 presets which assign the voices to the 61 keys - on a double-sided floppy disk of 573,440 bytes, of which the first 22 tracks may hold the operating system and the rest the bank. Like the Emulator, it writes a FM track format of its own, so a disk gets onto a computer only as an image of a HxC/Gotek floppy emulator (*.hfe*) or as a raw sector image (*.emuiifd*, *.img*). A bank is also read from a *bank file* (*.eii*), which holds the bank memory alone: the *Sound Designer for EII* file of the 1985 Macintosh software which received a bank from the sampler, as the EMXP tools extract and write it and as Arturia's Emulator II V imports it. The track format was described by the EMXP project; the bank layout was reverse-engineered here from the 90 disks of the factory library and 1,437 community disks and is described in *documentation/design/EMULATOR2_FORMAT.md*.

When reading, every preset of a bank becomes one multi-sample. A preset divides the keyboard (MIDI notes 36-96) into ranges which each play a voice at a transposition, and optionally a second voice, so the first voices become the first group of the multi-sample and the second voices the second group. From each voice the name, the loop (when it is switched on) and the audio are read, the latter expanded from the companded bytes which the sampler feeds to its AM6072 DAC; the root key of a range follows from its transposition. The loop is the end of a voice on the Emulator II: the audio in front of it plays once and the loop repeats, so a voice is converted up to the end of its loop. The settings of a voice are converted as well: the 4-pole low-pass filter with its cutoff, Q, keyboard tracking, envelope and velocity amount, the amplitude envelope with its velocity amounts on level and attack, the LFO with its rate, delay and its depths on pitch, filter and level, the fine tuning and the level. They were decoded by aligning the voice records with the SoundFont conversions which EMXP made of the same banks (see the design document); the chorus of a voice is not converted. The sampler saves 485,888 bytes of bank memory; a voice which was sampled into the memory behind that - which happens on a third of the factory disks - is cut there and noted, the rest of its audio is on no disk.

When writing, each multi-sample becomes one preset of the bank, several of them as a library on one disk: the zones of the first group become the voices of the key ranges, the zones of a second group their second voices, and identical audio is stored once. The audio is mixed to mono, re-sampled to the fixed 27,777 Hz of the sampler, companded and, since the sampler plays the loop as the end of a voice, cut at the end of its loop; the fine tuning, the level and the velocity to level amount of a zone are written into its voice, the other settings of the voices (envelopes, filter, LFO) are taken from a voice of the factory library (an unfiltered piano). The 485,888 bytes of bank memory hold about 17 seconds of audio; a voice which does not fit is shortened or skipped with a message. The operating system is not part of this program; it is copied from a system file (*.E2O*, as the EMXP project publishes them) or from any Emulator II disk image named in the settings. Without one the disk holds the bank alone, which the sampler loads once it has booted from another disk. Written disks have not been verified on hardware yet.
...
As a source, both file types are read. A multi-sample file becomes a multi-sample with the samples of its slot. A unified program file becomes one multi-sample which plays like the program: each oscillator of a part which plays a sample slot becomes a group with the level, tuning and panning of the oscillator and its part, on the keys of the keyboard sections which the part plays and shifted by the transposition of its keyboard. A program which plays a stereo sample with two parts panned to both sides, a program whose parts split the keyboard and a program which layers several sounds therefore each arrive as one instrument. An oscillator which does not follow the keyboard plays its samples with the same pitch on all keys. A slot of which the program does not play all samples is read as a multi-sample of its own as well, named after the program and the slot. Each sample becomes a zone with its key range, assigned note (root), fine tuning, volume, play range and loop including its cross-fade: *looping on when note on* becomes a loop which ends at the release of the note, *looping always on* a loop which continues. The wavetable and analog oscillators, envelopes, filters, modulation, effects, arpeggiator and sequencer of a program are not converted; a program whose parameters have an unknown layout is read as its sample slots. A sample which the device plays with 8 or 12 bit keeps the 16 bit of its recording, which is reported.

As a destination, a unified program file with its samples is written by default. Copy it into the *Programs* folder of the device, select the program location it shall replace and load it with *Import unified program data file* in the Global menu (*Bulk unified program data import* loads all files of the folder into consecutive locations); the device loads the samples into free sample slots. The program is the init program of the device ('init prog'), in which each part plays one sample slot with its first oscillator; the other oscillators are silent. A sample slot holds at most 8 samples: a multi-sample with more samples is split into several slots in the order of the keys, each played by a part of its own, and the parts split the keyboard between them. Samples which play at the same time - samples with the same assigned note or whose assigned note lies in the key range of another, e.g. the two channels of a stereo sample, which are panned to both sides, or a second sound which is tuned an octave down - are played by further parts, which get the panning of their samples. The tuning which the samples of a part share is set on its oscillator and their level on the oscillator and the part. Samples which a source doubles are played once with the sum of their volumes. A program has 4 parts: if its samples need more, the panned samples are played by parts in the center, and if that is still not enough, the layers which come last are not written; the two sides of a stereo sample keep the same keys. Velocity layers and round robins cannot be played by a program, only the loudest velocity layer and the first round are written; the multi-sample output writes them all. All samples of a program need to fit into the sample memory of the device, 35 seconds at 48 kHz; if they do not, the program is not written and the processing options can reduce the number of samples and their sample rate. The envelopes, the filter and the other settings of the source are not converted, the program keeps the ones of the init program.

Alternatively, multi-sample files are written, one for each sample slot. Copy them into the *Audio* folder of the device and load them with *Import multisample* or *Bulk multisample import* in the Global menu; the latter imports all files of the folder in alphabetical order into consecutive slots. Then select a slot with the wavetable control of an oscillator. Multi-samples with more than 8 samples, with layered samples or with more samples than fit into the memory are written as further slots, numbered after the name of the multi-sample.

For both, the samples are written as 16 bit mono (stereo samples are mixed down) with a sample rate between 10 and 48 kHz; other sample rates are converted. Coarse tuning moves the assigned note and the key range of a sample is limited to two octaves around it. A sample which plays the same pitch on all keys plays in tune on the key in the middle of its key range. In a program file the gain of a sample is stored as its volume, up to +6 dB, together with the volume of its part up to +10.6 dB; the multi-sample files have no volume, there the gain is applied to the audio. A gain which cannot be stored is applied to the audio only up to the peak level of the sample. Only the first loop is written, forwards and within the play range, and reversed samples are written reversed. Samples which play on the release of a note are not written. The written files are not yet verified on hardware.

### Destination Options

* Output: *Program with samples (.pgdata)* writes a unified program file, *Multi-sample slots (.bin)* one multi-sample file for each sample slot. On the command line: `-p ThirdWaveOutputFormat=pgdata` or `bin`.
...
The Kurzweil PC3 series (PC3, PC3K, PC3A, PC3LE, from 2007), the Forte and Forte SE, the PC4 and the K2700 share one object file format, the successor of the K2000/K2500/K2600 format. A file is a bank of numbered objects - programs, keymaps, samples, setups or multis, effect chains and more - with the extensions *.pc3*, *.p3k*, *.p3a*, *.ple*, *.for*, *.fse*, *.pc4*, *.p4s* and *.k27* by the device which wrote it (*.k2p* for the K2661 ROM compatibility samples of the PC3K); every device of the family loads the files of the others. Only the PC3K, the Forte family, the PC4 and the K2700 hold user samples, which a file stores as 16-bit sample objects (the Forte generation in a sample object of its own with 64-bit positions); the other devices play the ROM samples of the device, which are not present in their files. The layout follows the object viewer *Kurzweil Object View* by Brian Cowell (see *documentation/design/KURZWEIL_PC3_FORMAT.md*).

When reading, each program becomes one multi-sample: its layers reference the keymaps (one keymap or, for a stereo layer, two, which then play on the left and on the right side) from which the key ranges, velocity levels, root keys, tunings, volume adjusts, loops and the sample data are read. From the layers the key and velocity windows, the transposition, the amplitude envelope (unless the layer plays the 'natural' envelope of its samples), the velocity tracking of the amplifier and - if a layer uses one of the filter functions of the K2000/K2500/K2600 (the 1- and 2-pole low-pass, the 1-pole high-pass, the 2-pole bandpass and notch filters and the 4-pole filters) - the filter with its cutoff, resonance and filter envelope or cutoff velocity modulation are read. Layers which are switched off never play and are skipped. Keymaps and samples which are not referenced by a program are converted as multi-samples of their own. Programs and keymaps which map the ROM samples of the device - all programs of the PC3, PC3A and PC3LE and most factory sound packs - cannot be converted and are skipped with a note.

When writing, a file of the selected target device is created: a PC3K file (*.p3k*) with the sample objects of the PC3K, or a Forte, Forte SE, PC4, PC4 SE or K2700 file (*.for*, *.fse*, *.pc4*, *.p4s*, *.k27*) with the program layout and the sample objects of the Forte generation; every device of the family loads the files of the others. The PC3, PC3A and PC3LE have no user sample memory and are therefore no destinations. Each multi-sample becomes a program with one layer per group (up to the 32 layers of a program; further groups share the last layer). A layer plays a keymap with one sample object per zone and carries the amplitude envelope of its group; the velocity ranges of the zones are quantized onto the 8 dynamic levels of the keymap. The velocity sensitivity of the amplitude becomes the velocity tracking of the amplifier of the layer. A low-pass filter is written as the 1-pole low-pass (one pole) or the 2-pole low-pass with its resonance (two or more poles) of the devices, a high-pass filter as their 1-pole high-pass, a bandpass filter as their 2-pole bandpass and a notch filter as their 2-pole notch filter, each with the filter envelope or, if there is none, the cutoff velocity modulation. The samples are written in 16-bit with the zone gain in their volume adjust; the sample rate is kept up to 96kHz, the maximum sample playback rate of the devices. Since the device plays a loop until the end of the sample, the data after the loop end is cut off. If any zone of a layer is stereo, all samples of the layer are written as stereo pairs. The keymap covers MIDI notes 12-127 (C0-G9 in Kurzweil terms), keys below are dropped. Several multi-samples can be written into one file as a library; if the object IDs (1024-4095 per type) do not suffice, multiple files are created. Written files are not yet verified on hardware.

#### Destination Options

* Target Device: The device whose file is written - PC3K (*p3k*), Forte / Forte 7 (*for*), Forte SE (*fse*), PC4 (*pc4*), PC4 SE (*p4s*) or K2700 (*k27*). The selection sets the file extension, the program layout of the device generation and the type of the sample objects (the PC3K layout with 32-bit positions or the Forte generation layout with 64-bit positions). All devices of the family load the files of the others. CLI: `PC3TargetDevice=pc3k|forte|fortese|pc4|pc4se|k2700` (the extensions are accepted as well).
* Shorten the name for the device display: Keeps only the last separated part of the name (after " - ", " / ", " : " or " | "), e.g. *Greek Bazouki - Dark Tremolo* becomes *Dark Tremolo*. The name field of this format holds only 16 characters, so a source whose name is qualified with the bank it comes from otherwise loses exactly the part which tells its presets apart. The dropped part is not written anywhere else, therefore this is disabled by default. CLI: `PC3ShortenName=1`.
...
TAL-Sampler is an analog modeled synthesizer with a sampler engine as the sound source, including a modulation matrix and self-oscillating filters. Besides plain sample files a preset can use encrypted WAV files (*.talwav), in which the factory library and many sample packs deliver their samples and which the plug-in creates with 'Convert wav Samples to talwav'; an encrypted sample is decrypted when it is read. The name of a converted preset is the name of its file, which is also how TAL Sampler lists its presets; the name stored inside the file is the one under which the preset was saved once, and pack producers often rename the files afterwards. A sample is looked up at the path which the preset stores and, if it is not there, by its file name next to the preset and in the folders below it, since the stored path often belongs to the computer on which the preset was saved (e.g. `F:\Samples\Piano C3.wav`).

The envelopes are converted with the time law of the plug-in: a stage with the value x lasts 7.2 x^4 seconds, from an instant stage up to 7.2 seconds, whatever the length of the samples is (measured by rendering TAL-Sampler 4.7.2 offline). The amount of the filter envelope and the amount of an entry of the modulation matrix are bipolar, the middle of their range is no modulation. Of the modulation matrix the entries velocity to volume, velocity to cutoff, envelope 3 to pitch and modulation wheel to cutoff are converted. The volumes follow the laws of the plug-in as well: the volume of a sample is a linear amplitude factor (1.0 is +2.1 dB), the main volume and the volume of a layer are 40 log(2x) dB (0.5 is 0 dB, 1.0 is +12 dB), and all three apply to every zone. Since the volume of a sample reaches only +2.1 dB, a louder zone is written with the main volume and then the volume of the layers carrying the gain above it. The plug-in has one filter, through which each layer is switched or not: only the layers which are switched through it get the filter, and a written preset switches only the layers through it whose samples have a filter - a destination which knows only one filter for the whole preset writes none if the layers differ.
...
* Maximum number of layers: A layer holds up to 3 multi-samples, one on each oscillator, and the groups of the source which do not fit into the layers of the option are merged into the last of them. Those groups sound at the same time, but entries which overlap in one sample map alternate on successive notes instead, therefore a warning names the preset, the samples which are merged and the number of layers which keeps them apart. With 2 layers a source with more than 3 groups is written as 2 layers with up to 6 multi-samples, which keeps the groups apart and gives the second set of them its own filter and amplitude envelope; with 3 or 4 layers a source with more than 6 groups is written with up to 9 or 12 multi-samples - 3 leaves the fourth layer of an MK2 free for a sound of your own. Only as many layers as the groups need are used. Two layers are stored the same way by every instrument of the family, so such a preset plays on a Quantum, an Iridium and an Iridium Core as well as on the MK2 generation; a preset with 3 or 4 layers uses the layout of the MK2 generation (format version 15, four stored layers of which the unused ones are switched off), which only the Iridium MK2, the Quantum MK2 and first-generation instruments upgraded with the MK2 hardware can play - the first generation holds two layers. Only groups which sound at the same time are put into a separate layer - the device cannot select a layer by velocity, so a velocity split stays inside the sample maps. The default is 2 layers, which every instrument of the family plays; 1 layer keeps every preset in a single layer, e.g. to stack it with sounds of your own on the device, at the price of the alternating groups. Verified on an Iridium MK2: a written 2-layer preset loads and its second layer carries its own filter and amplitude envelope. The device imports only the samples which the first layer names, so the samples of the later layers are named in the last sample map of the first layer as well, in entries which never play - verified on an Iridium MK2 (OS 4.0.6): a test preset whose second layer brings a sample of its own plays both layers after the import, while the same preset without the entries fails to load that sample. Verified on an Iridium MK2 (OS 4.0.6) as well: a written 4-layer preset with one sample pitch per layer plays all four pitches, and a 3-layer preset with its switched-off fourth layer plays its three.
