...
Reading: every preset of a pack becomes one multi-sample. Each active layer becomes a group with its samples as zones: key range, root note, velocity range, sample start/end, loop with cross-fade (also ping-pong), reverse play-back, volume, panning and tuning. The layer volume, panning, fine tune, transpose and octave settings as well as the amplitude envelope (ADSR) of the layer engine are folded into its zones. The round robin layers which newer plug-in versions support are read as separate groups with round robin play logic. The fade-in/out of a sound and all the sequencer, arpeggiator and effects settings are not converted. The time unit of the envelope could not be verified, attack, decay and release values are interpreted as seconds.

Writing: each multi-sample source becomes one preset of the written pack (writing a library creates one pack which contains all the presets). Up to 4 groups are mapped to the 4 layers of the preset; since all layers play simultaneously and every zone keeps its velocity range this only makes a difference for sources with round robin groups, which the format does not support (they will play layered). More than 4 groups are merged into the first layer. Panning, gain and the fractional part of the tuning which is common to all zones of a layer is stored lossless in the layer parameters, the rest per sound (the tuning in full semi-tones, the gain as a percentage which cannot exceed 100%). The amplitude envelope of the first zone becomes the layer ADSR. To use the written pack, import it into Soundbox with the *Import* button of the preset browser.
