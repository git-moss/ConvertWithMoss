...
As a destination, a folder is created which contains the DWP file and all samples as 16-bit WAV files. FL Studio Desktop loads the DWP directly; for FL Studio Mobile copy (or ZIP) the whole folder into its user files. The DWP structure carries name, key/velocity ranges, root, gain, panning, loop, amplitude envelope and a low-pass/high-pass/band-pass/notch filter per zone.
