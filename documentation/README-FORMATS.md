...
As a source, DWP programs are read from their binary structure (see DIRECTWAVE_DWP_FORMAT.md in the design documentation): name, key/velocity ranges, root, gain, panning, loop and amplitude envelope of each zone (the sample chunks of the WAV files are used as the loop fall-back). Since the time law of the DirectWave envelope knobs could not be measured yet, the envelope times are approximations; the sustain level is exact. The samples are searched next to the DWP file and in the sub-folder named like the program. For monolithic files the embedded sample data is identified inside the program structurally (by its exact size); since no real monolithic file was available for verification, a report whether this works for your files is welcome. For files without the known binary structure (e.g. DWB banks) the mapping is reconstructed from the names of the sample files, which supports both the names written when sampling a plugin (e.g. *Piano 4xCycles_C5_100_2.wav*, round-robin cycles become groups) and the Automap tokens documented in the DirectWave manual (e.g. *sample_C4+KA3-F#4+V64-95.wav*). Note that DirectWave note names follow the FL Studio convention where the middle C (MIDI note 60) is called C5.

As a destination, a folder is created which contains the DWP file and all samples as 16-bit WAV files. FL Studio Desktop loads the DWP directly; for FL Studio Mobile copy (or ZIP) the whole folder into its user files. The DWP structure carries name, key/velocity ranges, root, gain, panning, loop and amplitude envelope per zone. Filters are not written (the filter type field could not be verified yet) and only the first round-robin cycle is kept since trigger groups cannot be written.
