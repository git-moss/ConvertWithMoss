...
ConvertWithMoss can extract Sampler and Simpler presets from ADV files as well as all instances of Sampler or Simpler in ADG files when selected as a source. This includes the presets and Live Sets of the Core Library which comes with Live. Most of the packs which are installed from Ableton, e.g. 'Orchestral Strings', cannot be extracted since their AIFF files use a proprietary encryption algorithm. It writes ADV files as the destination.

To convert the presets of the Core Library, select its folder in the installation of Live or one of its sub-folders. On macOS it is at `/Applications/Ableton Live <version/edition>.app/Contents/App-Resources/Core Library`, on Windows at `C:\ProgramData\Ableton\Live <version/edition>\Resources\Core Library`. Presets, Live Sets and Live Packs refer to the samples of the Core Library without containing them. Such a sample is taken from the Core Library which contains the source, otherwise from the Core Library of a Live installed in the standard location (on macOS also in `~/Applications`), a release version before a beta. This also works for presets which were copied out of the installation, whose absolute sample paths point to the machine on which Ableton built the library.
...
AIFF files are read with integer sound data, AIFC files with integer sound data in either byte order (e.g. the little-endian 'sowt') or with 32-bit floating-point sound data ('fl32'). Compressed AIFC files are not supported.
