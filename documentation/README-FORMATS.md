...
As a destination, a unified program file with its samples is written by default. Copy it into the *Programs* folder of the device, select the program location it shall replace and load it with *Import unified program data file* in the Global menu (*Bulk unified program data import* loads all files of the folder into consecutive locations); the device loads the samples into free sample slots. The program is the init program of the device ('init prog'), whose first oscillator of part 1 plays the samples at full level and whose other oscillators are silent. A sample slot holds at most 8 samples: a multi-sample with more samples is split into up to 4 slots in the order of the keys, each played by a part of its own, and the parts split the keyboard between them. Layered samples - samples with the same assigned note or whose assigned note lies in the key range of another, e.g. velocity layers and round robins - cannot be played by the program, only the first layer is written; the multi-sample output writes them all. All samples of a program need to fit into the sample memory of the device, 35 seconds at 48 kHz; if they do not, the program is not written and the processing options can reduce the number of samples and their sample rate. The envelopes, the filter and the other settings of the source are not converted, the program keeps the ones of the init program.
