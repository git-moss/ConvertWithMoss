...
* Waldorf Quantum/Iridium
  * Fixed: 'Snap loops to zero-crossings' did not remove the loop click when the source was not recorded at 44.1 kHz. The loops were snapped in the processing stage, but this format re-samples to 44.1 kHz afterwards, which moves every boundary off the zero-crossing it was snapped to. The up-sampling is now done first: 'always re-sample' is enabled, which the processing line of the log reports. Measured over one bank of the E-mu Xtreme Lead 1 library, whose samples are recorded between 25 and 32 kHz: across its 333 loops of 4096 frames or more the jump at the loop wrap drops from 4.57% to 2.14% of the sample level on average, the loops which jump by more than a tenth of the level fall from 63 to 15, 142 loops improve and 21 get worse. On the 'Shaku High', whose 0.9 second pad loop thumped once per repeat, that loop goes from 14.4% to 0.2%. A loop shorter than 4096 frames is still left alone, since snapping a loop of a few frames would change its pitch.
* Waldorf Quantum/Iridium
  * Fixed: A patch written with the second layer enabled did not play the samples of that layer. The device does not load a sample which only a layer beyond the first references - it reports 'loading samples/<patch>/<file>.wav failed' for each of them from a power-on state, so the volatile sample pool is not involved - while a second layer which only re-uses the samples of the first plays, because the first layer has already loaded them. Of the 104 two-layer patches which the E-mu Xtreme Lead 1 library produces, 85 brought samples of their own into the second layer and therefore did not play. Such a multi-sample is now written as a single layer, which keeps every zone: the groups which do not fit into the three oscillators are merged, which costs the panning of a group but no sample and no zone. The log says when this happens.
