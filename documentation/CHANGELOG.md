...
* Fairlight CMI 3
  * New: Voice files can now be written: either as a Series III voice (16-bit mono or stereo, up to 127 sub-voices with their key ranges, loops, tuning, gain and amplitude envelope) or as the fixed-size 8-bit voice files of the CMI I/II/IIx with their loop segments (one file per sample zone, e.g. for the QasarBeach recreation and the Arturia CMI V; the audio is re-sampled to the root frequency times 128, which is the pitch law of the CMI II - verified against QasarBeach). Written files have not been tested on real hardware yet.
