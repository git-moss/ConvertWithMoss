...
* New: Added support for the E-mu Emulator (1981), the first Emulator (HFE, EMUFD, IMG). One disk becomes one multi-sample: the zones of the lower and the upper half of the keyboard with their key ranges, root keys and fine tuning - which follow exactly from the pitch table of sample clock periods the disk stores for every key -, their loops, the three step low-pass filter setting and the audio, which is expanded from the companded bytes the sampler feeds to its AM6072 DAC. Writing creates a disk from a multi-sample: the zones are mapped onto the zone counts the sampler offers, every key gets the sample clock its source asks for, the audio is mixed to mono, re-sampled to 27,778 Hz and companded, and the operating system - which every disk of the machine carries and which is not part of this program - is copied from a system file or a disk image named in the settings. The format was reverse-engineered from the 79 disks of the factory library and verified against the SoundFonts which EMXP made of the 14 disks of the Production Set: key ranges, root keys, sample lengths and loops agree on every zone (see documentation/design/EMULATOR1_FORMAT.md). Written disks are not yet verified on hardware.
* Many thanks to Douglas Carmichael for plenty of contributions and fixes!
* New: Added support for the E-mu Emax and Emax II (EM1, EB1, EM2, EB2, EM1FD, EM2FD, EZ1, EZ2, IMG, ISO, HFE). A bank is a dump of the whole memory of the sampler and every one of its up to 100 presets becomes one multi-sample: the key map assigns each of the 88 keys a key area which plays a primary and optionally a secondary voice - the Dual Voice of the sampler - so the two become the groups of the multi-sample, and from each voice the root key, key range, tuning, attenuation, panning, delay, amplitude envelope with its velocity amount, low-pass filter with cutoff, Q, keyboard tracking, velocity amount and filter envelope, and the LFO with its vibrato and tremolo depths are read, together with the loop and the sample rate of its sample. The sampler packs those 48 parameters into the 32 bytes of a voice as a bit stream; the packing and the physical value behind each setting were resolved by aligning 37,965 voices of the Emax II library CD-ROMs with SoundFont files which EMXP produced from the same banks, and the ranges which fall out of it match the Emax manual exactly. Both samplers use the same structures and are read from the same files; which of the two a bank belongs to is read from the bank itself. The audio of the Emax is expanded from the companded bytes which the sampler feeds to its AM6072 DAC, the same transfer function the Emulator II uses, while the Emax II stores 16 bit. Banks are also read from floppy, hard disk and CD-ROM images, where every bank of the image is converted. Writing creates a bank from the sources, several of them as a library, with the parameters which are not decoded taken from a template of the factory library. The format was reverse-engineered from 45 Emax factory banks, the 147 banks of the Emax II library CD-ROMs, the owner's manual, the manual of EMXP and SoundFont conversions of those CD-ROMs which serve as a reference for the audio; see documentation/design/EMAX_FORMAT.md. Written banks are not yet verified on hardware.
