...
* New: Added support for the FL Studio DirectWave format (reading and writing of non-monolithic DWP programs with their key/velocity ranges, gain, panning, loops, amplitude envelope and filter; monolithic programs - one file per instrument with all samples embedded as FLAC compressed audio - are read and are written by default; the written program folder can be imported into FL Studio Mobile; the mapping of DWB banks and sampled plugins is reconstructed from the names of their sample files).
