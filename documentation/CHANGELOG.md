...
* E-mu Emulator III
  ...
  * Fixed: A malformed sample and a sample which a preset references but which is not in the bank were reported one line each. A bank which holds no samples at all - the sampler keeps its own code in one - therefore filled the log with a hundred identical lines before anything useful appeared. Both are now reported once with their count and their indices.
  * Fixed: The same bypass only applied when the resonance, the envelope, the key tracking and the velocity were all zero, so a parked cutoff which merely carries the default velocity or key tracking of its bank still created a filter - at the maximum of whatever the destination allows, where it does nothing but split an instrument into several modulation sets. Since only a negative envelope or velocity amount and the key tracking below the root key can lower a cutoff, those decide now. Across four library CD-ROMs this drops 19201 inaudible filters and keeps every one of the 10333 audible ones.
  * Fixed: Many banks of the library CD-ROMs - the Formula 4000 volumes, the General MIDI volume and a few banks of the classic volumes - were mastered with a tool chain which wrote the 16 bit sample number of a zone through 8 bits: the low byte is the true sample slot modulo 256, the high byte is zero or stale garbage. A preset of such a bank whose samples sit above slot 256 played completely unrelated material ('OBX Strings' of 'Vintage+InstrmtX' sounded basketball bounces), and a number whose garbage high byte survived pointed outside of the bank and lost the zone. The true slot is now inferred per preset from the note names which the E-mu sample names carry, the preset name occurring in the sample names and the feasibility of each 256-page against the sample table; a preset whose evidence is ambiguous keeps its stored numbers. Across the eight library CD-ROMs this repairs 4756 sample references in 821 presets of 32 banks and brings the missing-sample reports down from 641 to 37; the two clean classic volumes are read bit-identically to before.
  * New: The vibrato of a zone (the LFO routed to the pitch with its rate, delay and shape) is now read.
* E-mu Emulator IV
  ...
  * Fixed: The filter envelope was still read the way the amplitude envelope was before it was corrected: only the first stage of each pair and the level of decay 1 as the sustain. The operations manual describes the envelope as holding at the end of decay 2 until the key is released, so the sustain is that level. 7600 of the 8056 voices of the E4 factory library which use a filter envelope got the wrong sustain, 3743 of them a filter which stays open where it should close. Both envelopes now share one parser.
* Elektron Tonverk
  * Fixed: The computed key ranges of a multi-sample overlapped by one key at every zone boundary - the upper bound of a zone and the lower bound of the next one evaluated the same mid-point expression, so both zones owned the boundary key. A chromatic multi-sample double-triggered nearly every note with a copy transposed one semitone down; wider maps doubled each boundary key. The velocity ranges already subtracted the one step.
* Ensoniq EPS/ASR
...
* Ensoniq Mirage
  * Fixed: The oscillator mix cross-fade computed linear amplitudes but stored them as dB gains, which destroyed every level relationship - a muted second oscillator (linear 0) became 0dB = full volume. The cross-fade is now applied as a dB offset on top of the wave-sample level.
  * Fixed: The second-oscillator zone was added unconditionally, so every program gained a detuned phantom layer even when the oscillator mix was fully on OSC1; when the mix was active the zone was even added twice (to both groups).
  * Fixed: The envelope modulation depth was taken from the velocity-sensitivity parameter (where 0 means 'velocity has no effect'), so most programs lost their amplitude envelope entirely. The envelope now always applies and the parameter is read as what it is: the velocity modulation amount of the level (and of the filter cutoff).
* ISO/IMG file
  * Fixed: A disk image which uses the proprietary E-mu filesystem was only handed to the Emulator IV, so a CD-ROM of the older EIII/EIIIX/ESI samplers - which write the same filesystem - was rejected with 'No Emulator IV banks found'. Both formats are tried now and an image is only reported as empty when neither of them finds a bank. The filesystem is also named 'E-mu Emulator III/EOS' now instead of only 'E-mu EOS'.
* Korg KMP/KSF
  * Fixed: Writing a source with split-stereo zones sorted the zones into the left and right KMP by their tuning instead of their panning - the left KMP ended up empty and the right one received both channels.
  * Fixed: The sample start of a KSF file was masked to 12 bits instead of 3 bytes when reading, so starts of 4096 frames or more wrapped around (e.g. 5000 read as 904).
  * Fixed: The +12dB boost flag was applied as an addition of the linear powers (e.g. -6dB with the flag became +12.07dB instead of +6dB).
  * Fixed: The tuning was written into the single byte tune field without any range handling, so tunings beyond +-1.27 semitones wrapped around (+2 semitones became -56 cents). Whole semitones are now folded into the original key and only the remaining cents are written.
* Korgmultisample
