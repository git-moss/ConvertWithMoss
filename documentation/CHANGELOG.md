...
* New: Added support for the Kurzweil PC3 series, Forte, Forte SE, PC4 and K2700 (PC3, P3K, K2P, P3A, PLE, FOR, FSE, PC4, P4S, K27): the programs of these object files are read with their keymaps, user samples, envelopes, velocity tracking and filters, in the sample layout of the PC3K as well as in the one of the Forte generation; files of the PC3K (P3K), Forte and Forte SE (FOR, FSE), PC4 and PC4 SE (PC4, P4S) and K2700 (K27) are written. Written files are not yet verified on hardware.
...
* New: AIFF files with 32-bit floating-point sound data (AIFC 'fl32') are read, e.g. the sample 'Clave Synth' of the Core Library of Ableton Live, which its rack 'Percussion Spirit Kit' plays. They were rejected as if they were compressed.
* New: The Process dialog lists the loop options in the order in which they are applied: 'Snap loops which click' first, then 'Cross-fade loops' (formerly 'Set fixed loop-crossfade') with 'Only loops which still click' (formerly 'Only loops which click'). Together they snap the loops which click and cross-fade only the loops which snapping could not fix, which the dialog did not show. 'Only loops which still click' is available when a cross-fade above 0% is selected, since it does nothing without one. The log counts the loops which snapping 'moved' instead of 'adjusted' - a moved loop can still click - and the note about loops which still click after snapping names 'Only loops which still click' instead of the loop cross-fade, which cross-fades all loops. When loops still click although they were cross-faded, the note says that they start at the beginning of their sample, where there is no audio for a cross-fade.
* New: A loop only counts as clicking - for the note about clicking loops and for 'Only loops which still click' - if its loop point also breaks the curve of the waveform, which is how 'Snap loops which click' decides whether a loop clicks. The jump at the loop point alone also counted loops which wrap seamlessly through a steep part of the waveform, e.g. at the edge of a square or pulse wave, where the waveform moves that far from one frame to the next anyway. The snapping left such loops alone as clean, and 'Only loops which still click' then cross-faded them. After snapping, 55 loops of the 834 TAL-Sampler presets on the test machine and 5 loops of the 66 DX Dreams SoundFonts of Espen Kraft are no longer reported and cross-faded; 55 of these 60 loops bend at the loop point no more sharply than the audio around them does in its sharpest 1%.
* Fixed: 'Snap loops which click' could move a loop which does not click to a position which does. A loop whose last frame repeats its first frame bends a little at the loop point, which the snapping counted as a click, and the nearest zero-crossings of a pulse wave can be half of its level apart: a loop of the 'Prophet-5 Fat PWM' presets of the Hollowsun TAL-Sampler library, which repeats seamlessly, was moved by 4 frames to a position which jumps by 45% of the level at every repeat. A position which jumps at the loop point is not taken anymore for a loop which does not jump there. Of the 1,019 loops which the snapping moved in the 834 TAL-Sampler presets on the test machine, the 28 whose new position jumped stay where they are; the other moves and those in the 66 DX Dreams SoundFonts of Espen Kraft do not change.
...
* Fixed: The note about loops which click reported many loops of 24 and 32 bit samples which do not click, and the loop cross-fade for the loops which click was set on them. A jump at the loop point counts as a click when it is many times the normal movement of the waveform around it, but that movement was computed with integers, which overflow for loud 24 and 32 bit audio, and turned negative: every jump above 2% of the level counted. After snapping, the 834 TAL-Sampler presets on the test machine, whose samples are mostly 32 bit, now report 124 instead of 456 loops - nearly the 123 loops which they report when their samples are reduced to 16 bit, where the overflow cannot happen - and 117 instead of 449 loops get the cross-fade.
* Ableton
  * New: Samples of the Core Library which comes with Live are found for presets, Live Sets and Live Packs, also for presets which were copied out of the installation: in the Core Library which contains the source, otherwise in the one of Live installed in its standard location. They were searched like the samples of a project, relative to the preset, which missed the samples of 7 racks of the library, e.g. 'Borja Kit', and of its lesson sets.
  * New: Live Packs (ALP) are read: the Sampler and Simpler devices of all presets and Live Sets in a pack are converted with the samples from the pack, without unpacking it in Live first. The samples, which a pack stores compressed with FLAC, are decoded when they are read. With 'Create folder structure', the folders of a pack are recreated below a folder named like the pack. Presets which a pack stores in the binary format of older Live versions, e.g. in packs from 2008 and 2009, are counted and reported.
  * New: Live Sets (ALS) are read: every Sampler and Simpler on the tracks of a Live Set, also in racks, becomes a multi-sample named after the Live Set and its track.
  * Fixed: Presets which are stored as plain XML instead of compressed XML could not be read ('Not in GZIP format'), e.g. all 23 drum kits of the '80s hits' pack of Subsocial Studios.
...
* DecentSampler
  ...
  * Fixed: The settings of a group or of all groups were not used for their samples (e.g. velocity ranges), the pan of a group was added to the pan of its samples, and the volume of all groups was ignored.
  * Fixed: Envelope curves which are not set were linear instead of the default curves of DecentSampler.
  * Fixed: A linear envelope curve was not written, so DecentSampler played its default curve.
  * Fixed: An envelope which is the same in all groups was written to each sample, which left the envelope knobs of the user interface without effect.
...
* New: Added several new tags for better category detection.
...
* Fixed: Samples which are converted to the sample rate of the destination format (the conversions the re-sampling note reports) are now converted with the same band-limited resampler which the sample processing options use. The Java sound converter used before produced a slightly wrong number of frames - 88204 instead of exactly 88200 when a 2 second 48 kHz sample becomes 44.1 kHz - which stretches the audio against the sample and loop positions that are re-calculated with the exact rate ratio. A loop which wraps perfectly in the source then steps at its wrap-around point, the more the longer the loop is: measured on a 200 Hz sine with a 1.5 second loop, the step was 4.5% of the peak level and is 0.04% now. Up- and down-sampling also gain the band-limiting of that resampler, which the Java converter does not have.
* Fixed: An option which is enabled by default in the user interface was disabled when the conversion was started from the command line without giving that option. The command line only tested the value of a parameter, so an absent parameter always meant 'off', while the check-box of the same option is ticked in the user interface - the identical conversion therefore produced a different result depending on how it was started. Twelve options across eleven formats were affected, among them 'Re-sample to 16bit/44.1kHz' of the Waldorf Quantum/Iridium and 'Re-sample to 24bit/48kHz' of the Elektron Tonverk and the 1010music formats. Converting the Elektron Tonverk factory library to a Waldorf Quantum/Iridium preset from the command line wrote 1554 samples which the device cannot play: the library ships 64 bit float files and single-cycle waveforms whose pitch the Tonverk stores as a sample rate of up to 12 MHz, and without the re-sampling both were passed through unchanged. A parameter which is not given now keeps the default of its option.
* Backend
  * New: Added transpose, tuning, gain and panning attributes to performances (1010music bento/blackbox, Akai AKM, NI Kontakt 1, Roland S-7xx, TX16w, Yamaha Montage/MODX).
  * New: Improved setting of MIDI channels, with distinct values for OFF and OMNI for all performances.
  * Fixed: An error while one preset of a source file was converted ended the conversion of that whole file, so that every preset after it was silently lost - a disk image, a bank or a library delivers all of its presets from one read. The error is now reported with the name of the preset which caused it and with its stack, and the source is continued with the next preset. The same applies to performances.
* User Interface
  * Improved: The audition of the Contents dialog holds the note as long as the sound needs to show its character instead of fixed 2 seconds: through the delay, attack and decay of its amplitude envelope, dwelling on the sustain, or until the body of its longest sample was heard once (an evolving texture or a phrase which only loops near its end) - at most 6 seconds. A sound which decays to silence is held through its whole decay, so nothing of its natural tail is cut off; short one-shots still end at their sample end as before.
  * New: Contents dialog: A preset which shares its name with another preset in the same folder of the tree additionally shows the index which addresses it inside its file, e.g. 'Soul Ah's   (#12, 21 zones, C1-C6)'. Sample library CD-ROMs re-use a name for a variant of a preset more often than one would expect - the 'Emulator Standards' library of the E-mu Emulator III does so in 45 of its banks, where the bank 'Soul Ahs' holds two presets called 'Soul Ah's' which differ in their velocity sensitivity - and those rows were indistinguishable, so it was not possible to tell which of them to tick. The index is the one the exported list writes and by which the imported list selects a preset, so the tree and the list address the same preset. A name which is unique in its folder is shown unchanged.
...
* Akai S-1000/S-3000
  ...
  * Fixed: Zones with an upper velocity of 0 are skipped entirely. A velocity of 0 is a note-off, so such a zone can never be triggered - they mute the layers which belong to the partner program of a layered stack (and carry the copyright info, see above). Before, they became dead zones on the destination and the audition played them as always-on, which doubled the layers.
* Audiomodern Soundbox
  * New: Added reading/writing of category.
* Bliss
...
* DecentSampler
  * Fixed: The command line parameters 'DecentSamplerTemplateFolderPath' and 'DecentSamplerAddFilterToGroups' were read into each other's setting: the template folder path was taken from the add-filter parameter and the add-filter flag from the path. Giving a template folder on the command line therefore never reached the creator - it set the filter flag to false instead, since a path is not '1' - and giving the filter flag put the literal value into the template path. Neither option could be used from the command line; the user interface was not affected. The add-filter option now also keeps its default of enabled when its parameter is not given.
...
* Yamaha YSFC
  * Fixed: The play start, loop start and loop end of a written library still counted in frames of the source sample rate, although the audio itself is always converted to 44.1 kHz. For a 48 kHz source - e.g. every sample of a Korg Trinity KMP multi-sample - each position landed about 9% too far into the sample and the loop end that far beyond the end of the sample data, which clicks on every loop pass. The positions are now re-calculated to the destination rate, as the other creators with a fixed destination rate do.
