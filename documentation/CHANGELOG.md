...
* User Interface (thanks to Douglas Carmichael)
  * Fixed: The Settings and Processing dialogs opened with a bright white frame and, on macOS, repainted whenever the main window was clicked - both a risk for photo-sensitive (e.g. epileptic) users. The dialog scenes are now pre-filled with the theme background color on all platforms, which removes the white frame. On macOS the dialogs are additionally independent, non-modal, always-on-top windows, so clicking the main window no longer repaints them; they carry the base and dark-mode stylesheets directly (an unowned dialog does not inherit them from its owner) and the main action buttons are disabled while a dialog is open. On Windows and Linux the dialogs stay owned and modal.
