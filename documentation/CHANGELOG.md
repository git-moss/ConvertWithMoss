...
* New: Added support for the FL Studio DirectWave format (reading of DWP programs including loops, the amplitude envelope and the embedded samples of monolithic files, writing of non-monolithic programs; the written program folder can be imported into FL Studio Mobile; the mapping of DWB banks and sampled plugins is reconstructed from the names of their sample files). Written programs have not been tested in DirectWave yet.
