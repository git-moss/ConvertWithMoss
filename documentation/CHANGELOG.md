...
* New: Added support for the Roland MC-707/MC-101 project format (MPJ). The undocumented project container was reverse-engineered from Roland's own preset projects and the init-project image embedded in the device firmware (see documentation/design/MC707_FORMAT.md). A written project is the device's init project with the converted sounds in its user banks: a single-zone source becomes a user tone that plays its sample chromatically with the source's filter and amplitude envelope, a multi-zone source becomes a user drum kit mapping each key to its zone's sample with the transposition baked into the per-key pitch - both following the exact record patterns of Roland's own user-sample preset sounds. Samples are stored like the device's own import (interleaved stereo 16-bit / 44.1 kHz) with loop, level and original key in the sample table. Reading extracts every tone and drum kit that plays user samples, from the user banks as well as from the per-clip sounds (including Roland's downloadable preset projects). An option writes multi-zone sources as chromatic multisample tones (key map + tone, the FANTOM pattern on the shared tone record) instead of drum kits; it is off by default since no Roland-authored project uses the map table. Written projects are not yet verified on hardware (thanks to Douglas Carmichael).
* User Interface
