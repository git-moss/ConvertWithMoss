...
* E-mu Emulator III
  ...
  * Fixed: The two channels of a stereo sample are stored one after the other in either order, but the reader assumed the left channel always comes first. A sample stored right-channel-first was read at double length with mixed-up channels, or rejected as malformed when that overran the bank ('Vol. 1 - Emulator Standards' stores 16 such samples in 'Stereo Strings', '4 Piece Horns 8M' and 'Pop Ahs'; the string presets of the volume lost 2 of their 14 samples to that and every other one played garbage). Each channel is now read from its own start position.
  * Fixed: The original Emulator III stores the sample number of a zone in a single byte - the byte behind it is not part of it and holds unrelated values in many banks of the library CD-ROMs. It was read as a 16 bit number like in the later formats, which pointed at impossible slots; the heuristic high-byte repair caught most of them ('Full Arco String', 'Dance Club', 'Groupo Sinfonia'), but a reference whose low-byte slot is deleted stayed misreported (the number 3082 of 'Slow Strings' is a reference to the deleted slot 10). Emulator III banks now read only the low byte, which resolves all of these without any heuristics.
  * Fixed: A zone which references an empty sample slot - what deleting a sample leaves behind, the library CD-ROMs ship 23 such presets on 'Vol. 1 - Emulator Standards' alone - was reported as an error as if the reader had failed. The device finds nothing at such a slot either and keeps those keys silent, so this is now reported as a plain note instead.
* Kurzweil K2x00
