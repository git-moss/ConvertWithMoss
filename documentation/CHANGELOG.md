...
* New: Added support for the FL Studio DirectWave format (reading and writing of non-monolithic DWP programs with their key/velocity ranges, gain, panning, loops, amplitude envelope and filter; monolithic programs are read and can be written as well (all samples embedded as FLAC compressed audio); the written program folder can be imported into FL Studio Mobile; the mapping of DWB banks and sampled plugins is reconstructed from the names of their sample files).
